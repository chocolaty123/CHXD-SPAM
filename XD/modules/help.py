...
async def help(event):
    if event.sender_id in SUDO_USERS:
       await event.client.send_file(event.chat_id,
           ...
           [    
            Button.url("• ᴄʜᴀɴɴᴇʟ •", "https://t.me/XD_BOTX"),
            Button.url("• sᴜᴘᴘᴏʀᴛ •", "https://t.me/XD_N3TWORK")
           ],
           ],
           )
...
async def helpback(event):
   if event.query.user_id in SUDO_USERS:    
      await event.edit(
        ...
        await event.answer("Pahle jaake baap bol ʀᴏᴄᴋʏ ko wo tumhe sudo de dega", cache_time=0, alert=True)
